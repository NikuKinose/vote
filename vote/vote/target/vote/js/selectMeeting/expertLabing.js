...
function enterMeeting(id) {
    ...
    $.ajax({
        ...
        success:function (data) {
            console.log(data);
            // window.open("discussHomePage.jsp?linkIdFk=" +data.data.linkIdFk +"&linkType="+data.data.linkType +"&meetingId="+meeting.meetingId);
                ...
                success:function (data) {
                    ...
                    if(data.statusCode == 200){
                        ...
                        $.each(data.data, function (i, item) {
                            $("#meetingDiv").append(
                                "<div class='row'>" +
                                "<div  class='initContent col-xs-12'><img src=\"image/jj.jpg\" style=\"padding-top: 25px;padding-left: 0;float: left;\">"+
                                "<div style=\"margin:20px 60px; width: 20%; \"> <p>研讨室名称：" + item.meetingName + "</p>" +
                                "<p class='labcontent'>"+"研讨室简介："+item.meetingAbstract+"<a href='#' class='detail' >"+"详情"+"</a>"+"</p> </div>" +
                                "<div style=\"margin:10px 0;\"> <p>"+"研讨室ID:"+item.meetingId +"</p>"+
                                "<p>"+"研讨课题："+item.question.questionName+"</p>" +
